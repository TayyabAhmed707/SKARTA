...
public class Mutahar extends Boss
{
    ...
    public void render(Graphics g)
    {
        if (!renderLock)
        {
            g.drawImage(spritesheet[cycle][frame / Key.numberOfFramesRepeatedPerCycle], displayXcor, displayYcor, null);
        }

        if (isSpecialFeature)
        {
            g.drawImage(specialFeature[0][frame / Key.numberOfFramesRepeatedPerCycle],displayXcor - (shieldDimensions - width) / 2, displayYcor - (shieldDimensions - height) / 2, null);
        }
        for (Projectile projectile: projectiles)
        {
            if (!projectile.isLastFrameShown())
            {
                projectile.render(g);
                if(projectile.isDestroyed())
                {
