...
public class Handler implements Runnable
{
    ...
    Handler(String title)
    {
        ...
        Window window = new Window(Key.screenWidth, Key.screenHeight, title, camera);//new window created in which the game will be played
    ...
    void checkCameraMove()
    {
        if(!Player.collisionChecked)
        {
            ...
            if (Player.displayXcor > Key.screenWidth * 2 / 3 && camera.getXcor() < Key.absoluteHorizontalLimit)
            {
                ...
                camera.shift(movedBy); // move camera
                background.move(background.calculateSpeed(camera.getXcor(), movedBy)); // move background
