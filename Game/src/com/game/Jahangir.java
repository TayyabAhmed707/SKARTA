...
public class Jahangir extends Boss
{
    ...
    public void render(Graphics g)
    {
        if (isSpecialFeature)
        {
            g.drawImage(specialFeature[cycle][frame / Key.numberOfFramesRepeatedPerCycle], displayXcor - (attackDimensions - width) / 2, displayYcor - (attackDimensions - height) / 2 - 10, null);
        }
        else if (!renderLock)
        {
            g.drawImage(spritesheet[cycle][frame / 3], displayXcor, displayYcor, null);
        }
        if (!projectiles[0].isLastFrameShown())
        {
