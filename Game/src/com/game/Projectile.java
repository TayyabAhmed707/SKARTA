...
public class Projectile implements Animated
{
    ...
    public boolean isLastFrameShown()
    {
    ...
    {
        g.drawImage(spriteSheet[cycle][frame / Key.numberOfFramesRepeatedPerCycle], xCor, yCor, null);
    }
    ...
    void collide(int[][] foreground)
    {
        ...
        for(int i = 0; i <= noCollisionChecks; i++)
        {
            ...
            projectileXcorPath[i] = absoluteXcor - (int)(lastDx * (noCollisionChecks - i)/(noCollisionChecks * 1.0));
            projectileYcorPath[i] = yCor - (int)(lastDy * (noCollisionChecks - i)/(noCollisionChecks * 1.0));
            playerXcorPath[i] = Player.absoluteXcor - Player.effectiveDx + (int)(Player.lastDx * i/(noCollisionChecks * 1.0));
            if(Player.lastDx > 0)
            {
                if(playerXcorPath[i] > Player.absoluteXcor)
                {
                    playerXcorPath[i] = Player.absoluteXcor;
                }
            }
            else
            {
            ...
            playerYcorPath[i] = Player.displayYcor - Player.effectiveDy + (int)(Player.lastDy * i/(noCollisionChecks * 1.0));

            if(Player.lastDy> 0)
            {
                if(playerYcorPath[i] > Player.displayYcor)
                {
                    playerYcorPath[i] = Player.displayYcor;
                }
            }
            else
            {
